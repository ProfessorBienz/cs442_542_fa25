...
void std_seek_read(double* A, int n, int local_n, int first_n)
{
    int rank, num_procs;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &num_procs);
    ...
    int offset = n * num_procs * sizeof(double);
    int first_offset = n * rank * sizeof(double);
    fseek(file, first_offset, SEEK_SET);
    for (int i = 0; i < local_n; i++)
    {
        fread(&(A[i*n]), sizeof(double), n, file);
        fseek(file, offset, MPI_SEEK_CUR);
    }
    fclose(file);
